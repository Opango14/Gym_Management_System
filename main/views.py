...
def home(request):
	...
	return render(request, 'home.html',{'banners':banners,'services':services, 'trainers':trainers,'gimgs':gimgs})
...
def services(request):
	services=models.Service.objects.all()[:5]
...
stripe.api_key='sk_test_51RL2m5GhjWoiSIKDoUwWJH08k4ZYzNgoovQxFQtaYD1yi9d8kwyGtLOvtiVB8YHTqNK0Uv7S9fDJFne3ij1bo3K000Z2fZym3F'

def checkout_session(request,plan_id):
	plan=models.SubPlan.objects.get(pk=plan_id)
	usd_amount = plan.price
	kes_amount = usd_amount*100
	session=stripe.checkout.Session.create(
		...
		line_items=[{
	      'price_data': {
	        ...
	        'unit_amount': int(kes_amount*100),
	      },
	      'quantity': 1,
	    }],
	    mode='payment',
...
def pay_success(request):
	...
	from_email='sweatshop@gmail.com'
